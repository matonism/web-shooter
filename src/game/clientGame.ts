...
import { DEFAULT_SHOOTER_SETTINGS, type ShooterSettings } from "@shared/shooterSettings";
...
export interface RenderState {
  ...
  aimMode: ShooterSettings["aimMode"];
  bombRadius: number;
}
...
export class ClientGame {
  private localId = "";
  private inputSeq = 0;
  private keys = { up: false, down: false, left: false, right: false, bomb: false };
  private mouseWorld = new Vec2(0, 0);
  private firing = false;
  private shooterSettings: ShooterSettings = { ...DEFAULT_SHOOTER_SETTINGS };
  ...
  setTouchAim(aimX: number, aimY: number) {
    this.touch.aimX = aimX;
    this.touch.aimY = aimY;
    const len = Math.hypot(aimX, aimY);
    if (len >= 0.25) {
      this.touchAimAngle = Math.atan2(aimY, aimX);
  ...
  buildInput(): PlayerInput {
    ...
    if (this.touch.enabled) {
      ...
      if (this.shooterSettings.aimMode === "movement" && len >= 0.2) {
        angle = Math.atan2(dy, dx);
      } else {
        const aimLen = Math.hypot(this.touch.aimX, this.touch.aimY);
        angle =
          aimLen >= 0.25
            ? Math.atan2(this.touch.aimY, this.touch.aimX)
            : this.touchAimAngle;
      }
      fire = this.touch.firing;
    } else {
      if (this.shooterSettings.aimMode === "movement") {
        const len = Math.hypot(dx, dy);
        angle = len >= 0.2 ? Math.atan2(dy, dx) : this.predAngle;
      } else {
        angle = Math.atan2(
    ...
    this.inputSeq += 1;
    const bomb = this.keys.bomb || this.touch.bombPulse;
    this.touch.bombPulse = false;
    return { seq: this.inputSeq, dx, dy, angle, fire, bomb };
  }
  ...
  applyLocalPrediction(input: PlayerInput, dt: number) {
    ...
    const speed = this.shooterSettings.playerSpeed * this.localSpeedMult;
  ...
  applyServerSnapshot(world: ShooterWorldSnapshot) {
    ...
    this.shooterSettings = world.settings;
  ...
  getRenderState(): RenderState | null {
    ...
    const bombs: RenderBomb[] = remoteWorld.bombs.map((b) => ({
      id: b.id,
      x: b.x,
      y: b.y,
      team: b.team,
    ...
    return {
      ...
      bombs,
      powerups,
      aimMode: this.shooterSettings.aimMode,
      bombRadius: this.shooterSettings.bombRadius,
    };
  }
